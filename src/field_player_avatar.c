...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_UNDERWATER) && (heldKeys & B_BUTTON || FlagGet(FLAG_AUTO_RUN)) && FlagGet(FLAG_SYS_B_DASH)
     && IsRunningDisallowed(gObjectEvents[gPlayerAvatar.objectEventId].currentMetatileBehavior) == 0 && !FollowerComingThroughDoor())
    {
        FollowerPokeballSparkle();
        if (heldKeys & B_BUTTON && FlagGet(FLAG_AUTO_RUN)) { // if you hold b and auto run is enabled
