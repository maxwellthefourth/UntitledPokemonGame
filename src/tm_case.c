...
static void TMCase_ItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    if (itemId != -2)
    {
        if (!ItemId_GetHoldEffect(BagGetItemIdByPocketPosition(POCKET_TM_HM, itemId))) // (!ItemId_GetImportance(BagGetItemIdByPocketPosition(POCKET_TM_HM, itemId)))
        {
...
static void Task_SelectTMAction_FromFieldBag(u8 taskId)
{
    ...
    //show HM icon
    if (ItemId_GetHoldEffect(gSpecialVar_ItemId) == 1) // if (ItemId_GetImportance(gSpecialVar_ItemId)) -> changed because reusable TMs
    {
        DebugPrintf("price: %d", ItemId_GetPrice(gSpecialVar_ItemId));
